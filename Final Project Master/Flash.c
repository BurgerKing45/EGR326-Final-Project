...
//            parseString(TimeDate);      // parse the time and date string
//
...
void FlashDisplay(char Log[]){
    ...
    char entry1[20];
    snprintf(entry1, sizeof entry1, "%s:%s:%s %s/%s/20%s", hours, minutes, seconds, month, date, year);
    ST7735_DrawString(1, 8, entry1, ST7735_Color565(0xFF, 0xFF, 0xFF), 1, 0);

    /* parse second entry in the log, display on LCD */
    parseString(Log + 13);
    char entry2[20];
    snprintf(entry2, sizeof entry2, "%s:%s:%s %s/%s/20%s", hours, minutes, seconds, month, date, year);
    ST7735_DrawString(1, 9, entry2, ST7735_Color565(0xFF, 0xFF, 0xFF), 1, 0);

    /* parse third entry in the log, display on LCD */
    parseString(Log + 26);
    char entry3[20];
    snprintf(entry3, sizeof entry3, "%s:%s:%s %s/%s/20%s", hours, minutes, seconds, month, date, year);
    ST7735_DrawString(1, 10, entry3, ST7735_Color565(0xFF, 0xFF, 0xFF), 1, 0);

    /* parse fourth entry in the log, display on LCD */
    parseString(Log + 39);
    char entry4[20];
    snprintf(entry4, sizeof entry4, "%s:%s:%s %s/%s/20%s", hours, minutes, seconds, month, date, year);
    ST7735_DrawString(1, 11, entry4, ST7735_Color565(0xFF, 0xFF, 0xFF), 1, 0);

    /* parse fifth entry in the log, display on LCD */
    parseString(Log + 52);
    char entry5[20];
    snprintf(entry5, sizeof entry5, "%s:%s:%s %s/%s/20%s", hours, minutes, seconds, month, date, year);
    ST7735_DrawString(1, 12, entry5, ST7735_Color565(0xFF, 0xFF, 0xFF), 1, 0);

}
/* Reads time and date log from flash memory.
 * WhichLog = 1 for Alarm Log
 * WhichLog = 2 for Speed Log */
...
void WriteToFlash( char* InfoToWrite, uint8_t WhichLog){
    uint8_t i;      // current byte index in flash
    uint8_t currentFlashAlarm[65];   // string to store flash data
    uint8_t currentFlashSpeed[65];
